/*
 * 子機(XIAO ESP32C6)/親機(XIAO ESP32S3) 共有プロトコル定数
 * 詳細仕様: docs/protocol.md を参照。この値を変更する場合は必ず両方に反映すること。
 *
 * これは firmware/common/include/protocol.h の複製である。
 * (Windowsの一部ツールチェインが日本語+空白混じりの絶対パスへの -I 参照を解決できないため、
 *  プロジェクト自身の include/ に複製を置いている。canonicalな編集元は firmware/common 側。)
 */
#pragma once
